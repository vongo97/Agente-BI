...
async def process_simulation(sim_id: int, api_key: str, provider: str, mistral_key: Optional[str], selected_ids: List[int]):
    ...
    db = SessionLocal()
    try:
        print(f"[DEBUG] [Sim-{sim_id}] Iniciando simulación ({provider})...")
...
async def get_simulation(sim_id: int, db: Session = Depends(get_db)):
    sim = db.query(Simulation).filter(Simulation.id == sim_id).first()
    if not sim: raise HTTPException(status_code=404, detail="No encontrada")
    return sim
...
async def get_simulation_suggestions(req: SuggestionRequest, db: Session = Depends(get_db)):
    if not req.selected_ids: return []
    try:
        sources = db.query(DataSource).filter(DataSource.id.in_(req.selected_ids)).all()
        full_context_str = ""
        head_sample = ""
        ...
        for src in sources:
            ...
            if final_path:
                try:
                    ...
                    print(f"Error procesando {final_path}: {e}")
            else:
                print(f"ADVERTENCIA: No se encontró el archivo {src.name} en ninguna ubicación conocida.")
        ...
        try:
            ...
            print(f"Error en Generación de Sugerencias AI: {e}")
            return [{"title": "Error en Motor IA", "hypothesis": f"La IA no pudo procesar los datos: {str(e)[:100]}"}]
    except Exception as e: 
        print(f"Error Crítico en Sugerencias: {e}")
