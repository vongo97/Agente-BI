                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar">
                    ...
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                        <label className="flex items-center gap-2 text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-4">
                            <Zap className="w-3.5 h-3.5 text-blue-500" /> Configuración IA
                        </label>
                        <div className="bg-[var(--bg-tertiary)] p-1 rounded-xl border border-[var(--border-color)] flex gap-1 mb-4 shadow-inner">
                            <button 
                                onClick={() => setAiProvider('gemini')}
                                className={`flex-1 py-2 text-[9px] font-black uppercase tracking-tighter rounded-lg transition-all ${aiProvider === 'gemini' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Gemini
                            </button>
                            <button 
                                onClick={() => setAiProvider('mistral')}
                                className={`flex-1 py-2 text-[9px] font-black uppercase tracking-tighter rounded-lg transition-all ${aiProvider === 'mistral' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Mistral
                            </button>
                            <button 
                                onClick={() => setAiProvider('hybrid')}
                                className={`flex-1 py-2 text-[9px] font-black uppercase tracking-tighter rounded-lg transition-all ${aiProvider === 'hybrid' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Dual
                            </button>
                        </div>

                        {/* Input dinámico según la IA */}
                        <div className="relative group">
                            <input 
                                type="password" 
                                value={aiProvider === 'mistral' ? mistralKey : apiKey}
                                onChange={aiProvider === 'mistral' ? handleMistralKeyChange : handleApiKeyChange}
                                placeholder={aiProvider === 'mistral' ? "Mistral API Key..." : "Gemini API Key..."}
                                className="w-full bg-black/40 border border-[var(--border-color)] rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-blue-500/50 transition-all"
                            />
                            <div className="absolute right-3 top-3">
                                {(aiProvider === 'mistral' ? mistralKeyStatus : apiKeyStatus) === "success" ? (
                                    <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                ) : (aiProvider === 'mistral' ? mistralKeyStatus : apiKeyStatus) === "error" ? (
